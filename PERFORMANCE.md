...
`integration_test/open_dm_performance_test.dart`, `integration_test/release_journey_performance_test.dart`, and `integration_test/room_list_performance_test.dart` emit machine-readable frame data including refresh rate, frame budget, build/raster/total-span violations, worst timings, and raw per-frame timings. Release-journey output includes room-list scroll, composer keyboard, mixed rich timeline scroll, pagination, incoming-message insertion, and reaction/read-receipt/typing mutation records. The dedicated room-list target covers search/filter and Spaces/Sections switching against the deterministic 3,000-room fixture. Keep this output when investigating any failure.
