...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(data.label, contains('Muted room activity stays quiet.'));
  ...
  for (final entry in <String, Size>{
    ...
    testWidgets('${entry.key} survives 200% text scaling without exceptions', (
      ...
      if (entry.value.shortestSide < HomeScreen.phoneBreakpoint) {
        final alice = find.byKey(const Key('room-alice'));
        final visibleRoom = alice.evaluate().isNotEmpty
            ? alice
            : find.byKey(const Key('room-kite'));
        expect(visibleRoom, findsOneWidget);
        await tester.tap(visibleRoom);
        await tester.pumpAndSettle();
      }
  ...
  testWidgets('room rows expose a visible keyboard focus ring and activate', (
    ...
    await tester.pumpAndSettle();
    final nextTile = tester.widget<ListTile>(find.byKey(const Key('room-bob')));
    expect(
      FocusManager.instance.primaryFocus?.debugLabel,
      'Room bob',
      reason: 'Tab should keep desktop traversal inside adjacent room rows before entering the conversation pane.',
    );
