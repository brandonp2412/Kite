...
| Poll / location / media | Element X renders dedicated poll and map/location events plus a production full-screen media viewer. | Kite has deterministic poll choice/results cards and create/vote/end flows with fixed geometry, a full-screen media viewer with paging/captions/save/share hooks, and static/live timeline location cards with a composer location-share flow. Real Matrix poll/location ingestion, platform location/permission integration and production media-resolution evidence remain open. | `timeline_polls_{light,dark}.png`, `media_viewer_{light,dark}.png`, `timeline_locations_{light,dark}.png`, `location_share_{light,dark}.png`, `attachment_picker_{light,dark}.png`; compare with `TimelineItemPollView_*`, `TimelineItemLocationView_*`, and `MediaViewerView_*`. | Reviewed; gaps remain |
