...
- Thread-state coverage: Kite has deterministic light/dark renders for the normal thread, following state, failed-reply retry state, notification/deep-link focused reply, static thread-location replies, and room-list thread-unread decoration. Static location uses the same restrained fixed-geometry map card as the main timeline, stays within the outgoing reply bubble, and the thread attachment sheet intentionally omits the unsupported live-location action. The refreshed unread baselines include the same non-colour selected-room weight cue used by the main gallery.
...
- Poll and location gaps: Element X has dedicated poll choice/results geometry and map/location event cards. Kite now renders deterministic poll cards with fixed-height choices, selected/result progress, vote totals, and explicit ending/ended state alongside static and live-location cards with fixed geometry. The composer attachment grid opens a dedicated poll sheet for 2–6 choices as well as fixed/live location sharing. Poll vote/end mutations stay leaf-scoped and the new poll cards have deterministic light/dark reference goldens plus 120 Hz geometry coverage. Real Matrix poll event mapping/interoperability, locked profile evidence, platform geolocation/permission adapters, and Matrix-backed location transport remain open.
...
| Threads | Current `develop` has a dedicated thread top bar, timeline thread summaries, and a Labs Threads list with root/reply metadata. | Kite opens a dedicated thread route with root context, replies, pagination, composer, fixed-location replies, follow state, retry/focus states, room-list thread-unread decoration, and a room-scoped bounded Threads list with root/latest-reply metadata and unread counts. Live location is excluded from thread composition. SDK-backed list/location transport remains unfinished. | `thread_{light,dark}.png`, `thread_location_{light,dark}.png`, `thread_list_{light,dark}.png`, `thread_list_phone_{light,dark}.png`, following/retry/focus/unread variants; compare with `ThreadTopBar_*`, `ThreadSummaryView_*`, and `ThreadsListView_*`. | Reviewed; gaps remain |
...
| Poll / location / media | Element X renders dedicated poll, location, audio/voice and image/video content with a production full-screen media viewer. | Kite has deterministic poll choice/results cards and create/vote/end flows with fixed geometry, a full-screen image/video viewer with paging/captions/save/share hooks, static/live timeline location cards with a composer location-share flow, and compact fixed-geometry audio/voice playback cards with leaf-level playback state. Real Matrix poll/location/audio ingestion, platform location/audio integration and production media-resolution evidence remain open. | `timeline_polls_{light,dark}.png`, `media_viewer_{light,dark}.png`, `timeline_locations_{light,dark}.png`, `timeline_audio_card_{light,dark}.png`, `timeline_voice_card_{light,dark}.png`, `location_share_{light,dark}.png`, `attachment_picker_{light,dark}.png`; compare with `TimelineItemPollView_*`, `TimelineItemLocationView_*`, and `MediaViewerView_*`. | Reviewed; gaps remain |
