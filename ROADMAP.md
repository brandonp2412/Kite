...
- [ ] Add/remove/link rooms in Spaces.
  - Progress 2026-09-25: creating a private or public room inside a joined Space now carries the selected parent Space ID through the production room-creation boundary. The native Matrix bridge validates that the parent is an actual joined Matrix Space, then publishes reciprocal canonical m.space.parent and m.space.child state using the signed-in homeserver as the routing server. Linking or removing already-existing rooms remains pending.
