...
**Verification note (2026-09-15):** the Matrix Rust SDK boundary, encrypted SDK store, resumable incremental `/sync`, narrow presentation updates, lifecycle/recovery wiring, and automatic back-pagination are implemented with focused passing tests. Native sync now resumes from the persisted cursor, applies the configured timeline limit with lazy-loaded membership state, decodes sync/pagination payloads through an explicit background-isolate adapter, and uses bounded deterministic retry backoff. Presentation/restoration files now recover the last complete state across interrupted replacements, account removal drains writes and clears only that account's persisted cache, sync bursts coalesce full-cache persistence snapshots, equivalent event JSON preserves timeline Signal identity, startup lifecycle/connectivity bindings clean up failed attachments without dropping transitions, and unexpected sync-stream closure becomes a retryable failed state with native stop cleanup. The fast initial room-list portion of the current-SDK-sync checkbox remains open; migrating to simplified Sliding Sync also requires preserving an SDK position token across process death instead of weakening the existing cursor-restoration contract. Runtime-backed boxes remain unchecked because the repository quality gate currently reaches the deterministic test suite and then fails 18 pre-existing timeline/composer and Home gallery golden comparisons outside this producer lane. Rust formatting/tests/build, analyzer, focused Matrix tests, and the native ABI smoke stage pass before those unrelated golden failures stop the gate.
