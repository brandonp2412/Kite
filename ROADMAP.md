...
- [ ] Room-list filters equivalent to current Element X: All, Unreads, People/DMs, Rooms, Favourites, plus current upstream additions.
  - Progress 2026-09-25: Kite now matches current Element X compatible multi-select semantics for Unreads, People, Rooms, and Favourites while preserving the simplified home contract: focusing the fixed bottom search reveals a transient Filter chats action instead of restoring a permanent chip row or account-menu control. People/Rooms are mutually exclusive, compatible filters compose, active filters get a compact search-field badge, and clearing restores All. Current upstream's Invites filter maps to Kite's existing invite surface so invite cards remain out of the chat list. The profile-mode room_list_filter_change journey now targets this current interaction. Focused state/widget coverage and scoped analysis pass; keep unchecked until the required Waydroid profile/jitter gate is green.
...
- [ ] Space filter row with zero room-list reflow jitter.
  - Progress 2026-09-25: production room-list entries now derive direct Space membership from synced `m.space.child` relationships and the home sidebar exposes a fixed-height horizontal Space selector. Selecting All or a Space reuses the existing stable room signals/order and only changes visible membership; a widget regression verifies the room-list viewport rectangle does not move while switching Space filters, and the existing profile-mode `space_filter_change` journey now targets the same current selector. Focused tests, full Flutter analysis, and the full deterministic Flutter suite pass. Keep unchecked until the required Waydroid profile/jitter gate can run; Glass currently has no Waydroid ADB target.
...
- [ ] Space invitation preview with inviter, description, member context, and external/internal treatment where available.
  - Progress 2026-09-25: production invite sync now carries Space identity and Matrix join visibility through the Rust bridge, Dart codec, and persisted presentation cache. The account-sheet invite preview keeps inviter, description, and member context, adds Space visibility, and marks top-level/outside-hierarchy Space invitations as External while child Space invitations referenced by a joined Space remain internal. Focused UI, projection, codec, persistence, and all Rust bridge tests pass. Keep unchecked until the required Waydroid profile/jitter gate can run; Glass currently has no Waydroid ADB target.
- [ ] Browse/discover rooms in a Space, including rooms not yet joined.
  - Progress 2026-09-25: production Space browsing now calls the Matrix Space hierarchy endpoint through the Rust SDK bridge (native ABI 31), paginates direct hierarchy results, and merges unjoined rooms plus nested Spaces over the joined sync projection without polluting the joined Space rail. Public/restricted children use the production Join path, knock/knock-restricted children use Request, and invite/private children remain informational. Hierarchy overlays survive normal sync-cache reconciliation. Focused hierarchy/UI/adapter/native regressions, deterministic 120 Hz Space motion, full Flutter analysis/tests, all 15 Rust bridge tests, the 23-test real Dart/Rust ABI smoke suite, release audit, release APK build, and packaged native verification for armeabi-v7a, arm64-v8a, and x86_64 pass. Keep unchecked because the required Waydroid profile/jitter gate cannot run while Glass has no Waydroid ADB device.
- [ ] Create Space.
  - Progress 2026-09-25: the dedicated Spaces surface now creates private or public Matrix Spaces through the production room-management path. The native Matrix bridge emits an m.room.create creation content type of m.space, applies invite/public join semantics, and rejects encryption for Spaces. Production home passes its existing room coordinator into the secondary Spaces area, and deterministic widget plus 120 Hz motion coverage exercise the create sheet without adding permanent home chrome. Full Flutter analysis, the full deterministic Flutter suite, all 15 Rust bridge tests, the 23-test Dart/Rust ABI smoke suite, release audit, release APK build, and packaged native ABI/export verification for armeabi-v7a, arm64-v8a, and x86_64 pass. The locked Waydroid baseline still fails the global 16.666 ms frame contract on cold DM open (18.754 ms build, 36.754 ms raster, 55.907 ms total span), so keep this checkbox unchecked until that existing runtime-performance blocker is resolved.
- [ ] Edit/manage Space.
  - Progress 2026-09-25: the selected joined Space now reuses Kite's production room-settings stack for name/topic and related room settings, with the Spaces surface reconciling saved details immediately. Focused settings/widget coverage and the deterministic 120 Hz Spaces motion contract pass. Keep unchecked until the required profile-mode gate is green.
- [ ] Add/remove/link rooms in Spaces.
  - Progress 2026-09-25: creating a private or public room inside a joined Space carries the selected parent Space ID through the production room-creation boundary. Existing joined rooms can now also be linked from the Spaces area or removed from a Space. The production Matrix boundary publishes reciprocal m.space.parent/m.space.child state; unlink writes empty state content for interoperable removal. The new native operation is ABI 30, and focused UI/motion/adapter/native/Rust coverage passes. Keep unchecked until the required profile-mode gate is green.
...
- [ ] Nested Space navigation if present in current upstream baseline.
  - Progress 2026-09-25: joined m.space child relationships survive the production presentation projection, and the dedicated Spaces area now also discovers unjoined nested Spaces from the server hierarchy endpoint. Joined and discovered children support direct navigation plus parent links without recursive models; discovered Spaces are marked external and stay out of the joined Space rail until sync reports membership. Focused projection/hierarchy/widget coverage, deterministic 120 Hz child -> parent navigation, full Flutter analysis/tests, release audit, all 15 Rust bridge tests, the 23-test real Dart/Rust ABI smoke suite, release APK build, and packaged ABI-31 native verification pass. Keep unchecked because the required Waydroid profile/jitter gate cannot run while Glass has no Waydroid ADB device.
...
**Typing-indicator progress (2026-09-25):** production Matrix sync now consumes SDK m.typing ephemeral room events, excludes the local user, resolves member display names with user-ID fallback, and carries typing state through the Dart sync codec into a non-persisted leaf Signal. Existing timeline UI typing indicators now receive real production state while preserving fixed composer/timeline geometry. Focused Flutter coverage (56 tests), full flutter analyze, Rust formatting/check plus all 13 bridge tests, the release audit, native ABI smoke coverage (23 tests), and the deterministic Flutter suite pass. The typing-indicator checkbox remains unchecked because the required locked Waydroid jitter harness exceeded the available execution window and could not be completed in this run.

**Read-receipt progress (2026-09-25):** production Matrix sync now consumes public main-timeline m.receipt updates, excludes the local account, resolves member display names with user-ID fallback, and carries each reader's latest event through the Dart sync codec into non-persisted presentation state. Existing receipt avatars/details UI now receives real production state, and advancing a reader clears stale badges from earlier own messages without rebuilding unrelated timeline rows. Focused receipt/projection coverage (61 tests), full flutter analyze, all 13 Rust bridge tests, the release audit, the native ABI smoke suite (23 tests), and the full deterministic Flutter suite pass. The checkbox remains unchecked because the required clean locked Waydroid jitter baseline fails before artificial jitter is injected: observed raster frames exceeded the 16.666 ms budget (including 24.124 ms for send-message and up to 21.737 ms during formatting-toolbar motion).

**Read-marker progress (2026-09-25):** production room summaries now carry the Matrix SDK's m.fully_read event ID plus its client-side unread-message count through native sync, the Dart codec, and persisted presentation snapshots. Selected timelines map the fully-read position through raw Matrix event order to the first rendered unread message, including when the marker targets a non-rendered state event; retained-window fallback avoids conflating notification counts with unread-message counts. The existing fixed-geometry unread divider and jump control now receive production state, and optimistic mark-read state advances the marker to the latest known event. Focused marker/sync/store/projection coverage (72 tests), full flutter analyze, all 13 Rust bridge tests, the release audit, Waydroid lock self-test, native ABI smoke coverage (23 tests), and the full deterministic Flutter suite pass. The checkbox remains unchecked because the required clean locked Waydroid baseline fails before artificial jitter is injected: the cold-open raster frame reached 38.932 ms against the 16.666 ms device budget.
...
**Media-send progress (2026-09-26):** the composer now uses production platform pickers for files, images, videos, and camera capture, keeps selected bytes local through a removable pre-send preview, accepts captions, and uploads/sends Matrix media through the audited Rust SDK boundary with retry-safe local payload retention. Camera capture is exposed only on supported platforms and iOS carries the required camera usage description. Focused media/runtime/native coverage (62 Flutter tests), full Flutter analysis, the release audit, Rust formatting, and all 15 Rust bridge tests pass. The full quality gate reached the required locked Waydroid jitter stage after its earlier stages passed, but the clean 16.666 ms baseline failed on pre-existing DM/timeline journeys before the negative-control phase; no threshold was weakened. Keep Attach files, Pick images/videos, Capture media from camera, Media preview before send, and Captions on media unchecked until that runtime gate is green.

**Voice-message progress (2026-09-26):** the production composer now records microphone audio through the platform recorder, shows live waveform/duration feedback, supports cancel plus local preview/play/pause/seek before send, and carries duration/waveform metadata through the Matrix media-send stack so synced MSC3245 voice events project back as voice attachments. Android/iOS microphone permissions and the added recorder/player dependencies are covered by the release audit. Focused media/runtime/projection coverage (82 Flutter tests), Flutter analysis, Rust formatting, all 15 Rust bridge tests, and the release audit pass; the full quality gate advanced through its deterministic suite to Waydroid. The required clean 16.666 ms Waydroid baseline still fails before artificial jitter (cold-open raster 42.114 ms), so the voice-message checkbox remains unchecked and no performance threshold was weakened.
...
- [ ] Full-screen image viewer with smooth hero transition and no source-thumbnail flash.
  - Progress 2026-09-26: the shared media viewer now keeps the source thumbnail mounted while the visible full-resolution media genuinely fades from 0→1 using the existing Kite motion token; the previous AnimatedOpacity entered already at opacity 1 and therefore did not animate. Existing Hero source/target wiring remains intact. Focused media/profile coverage passes 29 tests, including deterministic 120 Hz viewer/avatar motion, and scoped analysis is clean. Keep unchecked until the required locked Waydroid/profile quality gate is green.
...
- [ ] Load full-resolution media only for the visible item.
  - Progress 2026-09-26: timeline, shared-content gallery, and thread media now open the shared paged viewer with the complete scoped media set and the tapped item as the initial page. Horizontal swipe plus desktop arrow-key browsing update the active item without wrapping, while full-resolution resolution is generation-cancelled and requested only after a page becomes current; adjacent pages retain thumbnails until selected. Focused timeline/thread/viewer coverage passes 29 tests, including deterministic 120 Hz adjacent-swipe geometry and explicit visible-only full-resolution assertions. Keep both boxes unchecked until the required locked profile/full quality gate is green.
...
- [ ] Share media internally and through the OS.
  - Progress 2026-09-26: the production media viewer now exports original-resolution Matrix image/video bytes instead of resized display thumbnails, including encrypted media through the audited Rust SDK boundary. Android 10+ saves into public Downloads through MediaStore, desktop uses the native save-location picker, web/older Android fall back to the platform export surface, and OS sharing preserves captions while cancellation stays silent. Local pending-media bytes are reused without a redundant Matrix download. Focused Flutter coverage, full analysis/deterministic tests through the quality gate, the release audit, all 16 Rust bridge tests, the native ABI smoke stage, an x86_64 Android bridge rebuild, and a debug x86_64 APK build pass. The required locked Waydroid baseline did not complete within the harness's five-minute timeout, so these boxes remain unchecked.
...
- [ ] Date/time/number formatting by locale.
  - Progress 2026-09-24: the shared locale formatter now drives synced timeline times and date separators, invite/Space member and room counts, room-list unread badges plus accessibility labels, support-storage and Matrix-media sizes, Matrix-media durations, license/device counts, media position counters and semantics, thread/reaction/read-receipt counts, forwarding counts, typing-user overflow, and room-key import counts. Matrix attachment size/duration metadata remains structured until presentation instead of being preformatted in the controller. German regressions verify a four-digit unread count renders as 1.234 rather than 1,234, a 1.5 KiB value renders as 1,5 KiB, and Matrix media renders 2,1 MB. Full analysis is clean and the full Flutter suite passes 1,144 tests with 5 intentional native-library skips. The user-facing dynamic-number audit is code-complete; keep unchecked until the required runtime quality gate is green.
...
**Waydroid raster-gate progress (2026-09-25):** the primary \`open_dm_performance_test.dart\` target now installs the same \`KiteShaderWarmUp\` used by production startup and the authentication performance target, so the jitter harness no longer benchmarks an artificially colder shader path. A locked Waydroid x86_64 profile baseline still fails the unchanged 16,666 µs raster contract: cold DM open reached 42,538 µs, 30 warm room switches peaked at 17,658 µs with 10 raster violations, formatted send reached 57,567 µs, and forwarding reached 37,822 µs; send, formatting-toolbar, mention-autocomplete, and edit journeys stayed within the raster budget, and the recorded journeys had zero build-budget violations. Keep runtime-gated roadmap boxes open; no threshold was weakened.
