...
- [ ] Date/time/number formatting by locale.
  - Progress 2026-09-24: the shared locale formatter now drives synced timeline times and date separators, invite/Space member and room counts, room-list unread badges plus accessibility labels, support-storage byte sizes, license/device counts, media position counters, thread/reaction/read-receipt counts, forwarding counts, and typing-user overflow. German regressions verify a four-digit unread count renders as 1.234 rather than 1,234 and a 1.5 KiB value renders as 1,5 KiB. Full analysis is clean and the full Flutter suite passes 1,143 tests with 5 intentional native-library skips. Keep unchecked until remaining context-free formatted values such as decrypted-key import status and attachment metadata are moved to presentation-time locale formatting, and the required runtime quality gate is green.
