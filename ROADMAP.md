...
**Profile progress (2026-09-17):** own-profile loading and display-name mutation reach the audited Matrix Rust SDK through the production account runtime, and the existing account identity row opens the profile as a secondary screen without adding home-screen chrome. Target-user profile loading and open/create-DM primitives are production-backed through the same boundary. Production avatar editing now uses the cross-platform system file selector, uploads validated image bytes through the Matrix SDK media API off the Flutter UI isolate, and applies the returned MXC URI through the existing profile mutation flow; cancellation and upload failures remain contained in the secondary profile screen. The native boundary is ABI 19, Android JNI libraries were rebuilt for arm64-v8a, armeabi-v7a, and x86_64, and a release APK contains all three updated libraries. The dependency/license baseline was re-reviewed for the selector stack and passes with 51 hosted packages. Focused profile/runtime/native tests pass, scoped analysis is clean, and `tool/quality_gate.sh` passes release audit, Rust/ABI verification, full analysis, and all 1,026 deterministic Flutter tests with 3 intentional native-library skips before stopping only at the required Waydroid jitter stage because Glass has no ADB device. Profile checkboxes remain unchecked until that device gate passes and production MXC avatar rendering is wired instead of falling back to initials.
