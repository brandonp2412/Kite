...
**Verification note (2026-09-15):** the Matrix Rust SDK boundary, encrypted SDK store, resumable incremental `/sync`, narrow presentation updates, lifecycle/recovery wiring, and automatic back-pagination are implemented with passing focused tests. Native cold-start sync uses a deliberately small initial timeline payload until the first cursor is established, retries keep that lightweight shape, and resumed/offline sync immediately uses the full timeline depth so message bursts are not truncated. Sync/pagination FFI and JSON decoding stay behind background-isolate adapters, retry backoff is bounded and deterministic, and leaf-only room metadata changes preserve the room-order Signal identity instead of re-sorting the whole list. Presentation/restoration files recover the last complete state across interrupted replacements, sync bursts coalesce bounded presentation snapshots, and account teardown now cancels runtime sync before closing the SDK boundary, clears abandoned presentation writes, and makes outbox shutdown terminal. Unexpected sync-stream closure resets the underlying engine before retry so a new subscription cannot falsely report running without restarting native sync. The repository gate passes the Waydroid-lock self-test, Rust formatting/tests/build (6 tests), Flutter analysis, the real native ABI smoke suite (8 tests), and all 352 deterministic tests with 3 intentional skips. Runtime-backed boxes remain unchecked because the required locked Waydroid baseline still violates raster budgets outside this producer lane: cold DM open had 1 raster violation (51,250 µs), reply had 4, copy had 4, delete confirmation had 1, and edit commit had 2; warm DM, send, and delete-redaction build/raster budgets passed. The current SDK path therefore has verified foundation behavior, but ROADMAP completion remains gated on the repository-wide performance contract.
