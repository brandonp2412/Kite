...
Reverified on 2026-09-26 after adding voice-message recording and local playback: the executable audit passed with all 111 hosted packages accounted for (86 BSD-family, 13 Apache-2.0, and 12 MIT).

The reviewed `pubspec.lock` SHA-256 is:

`bc2bb4046d1cf288204462086c4a96f27137c34370a21aadbceb653afdc61c71`

The lock contains only SDK or hosted packages. The executable audit rejects Git/path dependencies and requires a non-empty license or copying file for every hosted package resolved by the lock. It recognizes only the reviewed BSD-family, Apache-2.0, and MIT license texts; unknown license text fails the gate for manual review. The current baseline contains 111 hosted packages: 86 BSD-family, 13 Apache-2.0, and 12 MIT. The added `intl` 0.20.3 dependency is BSD-family and is required by Flutter's generated localisation/plural support. The later `ffi` 2.2.0 direct dependency is also BSD-family and is required by the Matrix Rust native bridge. The `file_selector` 1.1.0 dependency and its resolved cross-platform selector stack add 13 hosted packages for production avatar selection: 12 BSD-family and one Apache-2.0 (`file_selector_android`). The `url_launcher` 6.3.2 dependency and its resolved cross-platform launcher stack add 8 BSD-family packages for opening timeline web links through the platform browser. The `share_plus` 13.3.0 dependency and its resolved cross-platform sharing stack add 27 hosted packages for the production system share sheet: 25 BSD-family and 2 MIT. The `image_picker` 1.2.3 dependency and its resolved camera stack add 9 hosted packages for composer camera capture: 6 BSD-family and 3 Apache-2.0 (`image_picker`, `image_picker_android`, and `image_picker_ios`). Voice-message recording and local preview add `record` 7.1.1, `audioplayers` 6.8.1, and direct `path_provider` 2.1.6 usage; the newly resolved recorder stack contributes 8 BSD-family packages while the player stack plus `synchronized` contributes 8 MIT packages. All resolved package license files match the executable audit's reviewed license families.
