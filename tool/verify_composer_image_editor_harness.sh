...
printf 'Composer image-editor profile gate on %s: must PASS\n' "$device"
timeout --signal=TERM --kill-after=10s 5m \
  ...
  --target=integration_test/composer_image_editor_performance_test.dart \
  -d "$device"

printf '%s\n' 'Composer image-editor profile gate passed.'
